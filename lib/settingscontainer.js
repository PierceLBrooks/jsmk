...
//
//  FrameworkList: like LibList, but triggering special per-framework logic to
//      produce searchpaths and linkage.  Some frameworks are portable (eg OpenSSL)
//      and some are native (eg MacOS kits). Frameworks can include versioning 
//      requests (eg gtk2 vs gtk3).
//
...
class SettingsContainer
{
    ...
    GetEnv(key)
    {
		if (util.BuildPlatform != "win32")
        {
            return this.EnvMap[key];
        }
        for(let k of Object.keys(obj))
        {
            if (key.toLowerCase() == k.toLowerCase()) return this.EnvMap[k];
        }
        return null;
    }
    ...
    GetDependencies(syntax)
    {
        let result = [];
        if(!this.DepList) return result;
        if(!syntax) return this.DepList;
        let slist = syntax.split(" ");
        for(let i=0;i<this.DepList.length;i++)
        {
            let val = this.Interpolate(this.DepList[i]);
            for(let s of slist)
                result.push(jsmk.Interpolate(s, {VAL: val}));
        }
        return result;
